...
class Database {
    ...
    /**
     * Escapar string (aunque PDO ya lo hace)
     */
    ...
    private function logError($exception, $sql, $params) {
        $errorMsg = date('Y-m-d H:i:s') . " - Error de base de datos\n";
        $errorMsg .= "Mensaje: " . $exception->getMessage() . "\n";
        $errorMsg .= "SQL: " . $sql . "\n";
        $errorMsg .= "Params: " . json_encode($params) . "\n";
        $errorMsg .= "-----------------------------------\n";

        error_log($errorMsg, 3, BASE_PATH . '/logs/database_errors.log');
